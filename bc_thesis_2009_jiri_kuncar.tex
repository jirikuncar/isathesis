...
tak z hledika její vlastní realizace. Důležitou roli zde hraje plánování
časového hramonogramu a jeho dodržování.

Zlepšení výsledků činnosti firmy se nemusí dostavit okamžitě po spuštění
nového systému, ale až po adaptaci všech zúčastněných stran. Výhodou
je pokud byl daný systém navržen v souladu se zaběhnutými a fungujícími
firemními procesy. Proto bude práce věnovaná především jejich důkladnému
popisu, na kterém bude dále stavět.


\section*{Cíle práce}

Cílem práce je návrh a základní implementace webového informačního
systému usnadňující činnost a spolupráci více nezávislých subjektů
za použití vhodných opensource technologií a frameworků v použitých
programovacích jazycích. V práci budou rozebrány funkce systému pro
správu, uržování a sdílení informací v prostředí internetu.

Implementovaný systém bude mimo jiné obsahovat prostředky pro správu
uživatelů, evidenci zakázek a fakturaci služeb. Cílem práce není,
vzhledem k náročným právním úpravám a častým změnám, vytvořit komplexní
účetní program, ale pouze systém úmožnící tvorbu přehledů pro účetní
evidenci, které mohou být zavedeny do některého již existujícího účetního
programu.
...
Protože ačkoliv jsou XML databáze v komerčních produktech na velmi
dobré úrovni, jejich implementace ve volně dostupných aplikacích mírně
zaostává.%
\footnote{@todo\href{http://www-01.ibm.com/software/data/db2/express/}{http://www-01.ibm.com/software/data/db2/express/}%
}
...
\item \emph{Způsob platby }a\emph{ typ smlouvy} úmožňují generování účetních
přehledů.
\end{itemize}
...
Nespornou výhodou používání frameworků spočívá jednodušším vývoji
aplikací a minimalizací rizika chyb v jinak ručně psaném jádru aplikace.
Toto je zajištěno pouze pokud má kvalitní a úplnou dokumentaci a je
zastřešen silnou komunitou nebo společností zajištující jeho vývoj.
Některé frameworky rovněž dokáží do jisté míry zakrýt rozdíly mezi
ve verzích. Děje se tak, za cenu zpomalení některých částí aplikace,
díky kotroly verze překladače a vykonáním alternativního kódu.
...
CakePHP zastává programátorskou filosofii \emph{DRY} - Don't Repeat
Yourself. Ta zdůrazňuje, že jednotlivé části kódu by se v programu
neměly opakovat, jelikož se snižuje srozumitelnost kódu a při modifikaci
se musí upravovat stejný kód na více místech, což často vede k nesnadno
odhalitelným chybám programu. K tomuto účelu se používají \emph{,,Pomocníci''}
(\emph{Helpers}), kteří daný kód generují. Při potřebě změny je lze
snadno upravit, bez potřeby zásahu do míst odkud jsou voláni.
...
aplikací, kde je součástí platformy označované jako \emph{LAMP}, která
obsahuje \emph{Linux}, \emph{Apache}, \emph{MySQL} a \emph{PHP}. Obecně
jednotlivé části mohou být zaměněny (např. PHP za Perl nebo Python,
MySQL za Postgres) při zachování zkratky, která je zaužívaným označením
pro operační systém, webový server, databázový systém a skriptovací
jazyk sloužící k obsluze a generování webových stránek.
...
zakryta třídou \emph{Overloadable}. Ta navržena tak, aby zakryla rozdíly
mezi PHP verze 4 a 5. 

Modely jsou determinovány návrhem databáze. Relace mezi jednotlivými
...
a mají své vlastní nastavení. Relace 1:1 (\emph{hasOne}), 1:N (\emph{hasMany})
a N:1 (\emph{belongsTo}) mají většinu nastavení společnou:
\begin{description}
...
\end{description}
Při modelování relace M:N (\emph{hasAndBelongsToMany - HAPTM}) je
potřeba nadefinovat tabulku (\emph{joinTable}), přes kterou bude prováděno
...
být jedním z následujích:
\begin{description}
\item [{all}] najde všechny data splňující podmínky definované ve druhém
parametru
...
v proměnné \emph{\$displayField} daného modelu indexované standardně
primárním klíčem
...
můžete nalést na\href{http://dev.mysql.com/tech-resources/articles/hierarchical-data.html}{http://dev.mysql.com/tech-resources/articles/hierarchical-data.html}.%
}
...
\begin{lstlisting}[basicstyle={\small\sffamily},tabsize=4]
array(
...
jenž jsou v řetězci asociací dále, bylo výchozí chování parametru
nedostatečné. Naštěstí existují doplňky, které takové činnosti úmožňují.


\subsection{Rozšíření chování (\emph{Behaviors})}
...
množství dotazů, které mohou mít vliv na výkon dané aplikace.%
}. Je to ale pouze syntaktický cukr k metodám \emph{bindModel()} a
\emph{unbindModel()}, které úmožnují definovat pouze asociace příslušící
danému modelu. Containtable přidává metodu \emph{contain()}, jenž
je volána i pokud je nalezen index ,,contain'' v poli s vyhledávacími
parametry metody \emph{find()}. Ta se postará o dynamickou asociaci
potřebných modelů.
...
Proto je použité rozšíření \emph{,,Linkable}''\cite{linkable},
které má obdobnou syntaxi nastavení, ale vytváří jeden SQL dotaz za
pomocí klauzují JOIN.

Pro usnadnění práce s relacemi typu M:N přes tabulku s klíči je použito
rozšíření \emph{,,Extend Associations''\cite{ExtendAssoc}} . To
úmožňuje jednoduché přidání nebo smazání asociací, bez ovlivnění ostatních
již existujících. K tomu slouží nově definovaná sada metod:
...
\item [{habtmDeleteAll(\&\$model,~\$assoc,~\$id)}] smaže včechny asociace
k danému záznamu modelu
\end{lyxlist}
Pro překlady do cizích jazyků bylo použito rozšíření \emph{Translate},
které úmožňuje nadefinovat seznam sloupců tabulky, které mají být
překládány. Přeložená data jsou ukládána v oddělené tabulce (\emph{i18n}
- lze změnit v konfiguraci pro každý model) a při dotazu je vytořeno
spojení s touto tabulkou pomocí složeného klíče - jméno modelu, cizí
klíč do daného modelu a jméno překládaného sloupce.
...
volána pokaždé, když metoda \emph{DboSouce::execute()} vrátí chybu.
V našem případě je chyba zpracována a chybová hláška je uložena pro
pozdější prezentaci uživateli.
...
Řadiče jsou používany ke správě aplikační logiky. Pro možnost používat
...
v app/config/core.php v interní proměnné Security.salt. Tato vlastnost
byla v systému na obtíž, hlavně kvůli přenositelnosti již zadaných
hesel v předchozí aplikaci. Proto byla vytvořena komponenta Authmd5,
která nepřidává zabezpečovací řetězec k heslu. Další drobnou změnou
oproti původní komponentě je automatické hashovaní hesla z formuláře
...
definovaný v konstaně \emph{DEFAULT\_LANGUAGE} v souboru \emph{app/config/core.php}.\cite{p28n}
\item [{Password~Helper}] Vygeneruje ,,hezké'' heslo zadané délky bez
opakujících se znaků. Jednotlivé znaky jsou vybírány z atributu\emph{
possible} dané komponenty.\cite{pass-gen}
\item [{Autocomplete}] Přidává metodu \emph{autocomplete()}, která zpracuje
získaná data z formuláře (data{[}Model{]}{[}položka{]}), vyhledá vyhovující
...
týkat. Následující příklad je vybrán z řadiče produktů, kde má být
zákazníkům úmožněno vyhledávat pouze v produktech, které se jich přímo
týkají.
\end{description}
\inputencoding{latin2}
...
podpory javascriptivé knihovny \emph{control.js}, která je součástí
balíku \emph{scriptaculous.js}.
\item [{Filter}] Komponenta pro validaci vyhledávacích formulářů a vygenerování
podmínek pro vyhledávání.
\item [{Email}] Slouží k odesílání emailů. Lze používat šablony, html zprávy
a přidávat přílohy.
...
Při vytváření základní kostry byla použita ,,pekárna'' kódu, jejiž
...
Ty jsou zajitěny komponentou \emph{Auth}, která je nastavena v řadiči
aplikace (\emph{AppController}) v metodě \emph{beforeFilter()}.
\begin{description}
\item [{login()}] Po přihlášení je aktualizována položka ,,poslední přihlášení''
v databázi, nastavena zpráva uživateli a uživatel je přesměrován na
požadovanou nebo výchozí stránku.
\item [{logout()}] Smaže cookie, nastaví zprávu o úspěšném odhlášení a
přesměruje na výchozí stránku.
\item [{account()}] Slouží ke změně hesla uživatele. Pokud jsou poslány
data a hesla projdou validací, je uživateli nastaveno nové heslo.
...
rozšířit možnost obnovy hesla o kontrolní otázku.
...
\begin{description}
\item [{admin\_account(\$id)}] Úmožní administrátorovi změnit heslo uživateli
...
tabulek uživatelů, slmuv, výkazů, produktů, údálostí a přihlášek daného
...
vyplněného filtru.
\item [{admin\_shedule()}] Pro každý vyfiltrovaný produkt vyhledá dny v
týdnu, ve kterých probíhá výuka a přidá informaci o časovém intervalu.
\item [{admin\_shedulePdf()}] Použije výše uvedenou metodu k nalezení dat.
...
a~vygeneruje přehled pro sestavení výplat a kontrolu přijatých faktur
...
if typ slu�by == slu�ba then
	if zp�sob platby == hotov� then
		zaokrouhli v�platu na 50 hal���
	endif
else if pl�tce dan� then
		if v�plata > 5000 then
			z�lohov� da� := 15% z v�platy
		else
			sr�kov� da� := 15% z v�platy
		endif
endif
\end{lstlisting}
...
\subsubsection{Příjmové doklady}
...
položky a celkový počet položek k zaplacení. Rozdíl těchto dvou mezivýsledků
určuje nedoplatek popřípadě přeplatek na daném produktu. Jelikož je
rozhraní pro přidávání a mazání položek z dokladu postaveno na AJAXu
jsou vytvořeny dvě metody \emph{admin\_additem()} a \emph{admin\_delitem()},
které po provedení akce vygenerují element s aktualizovanými položkami
dokladu.


\subsubsection{Události v kalendáři}

Pro pro přehledné zobrazení údálostí jsou na výběr dvě možnosti. Tou
první je zobrazení všech událostí v daném měsíci v tabulce podobné
kalendáři. K tomuto účelu slouží metoda \emph{admin\_calendar()},
kterou jsou získány všechny záznamy vyhovující filtru a pro generování
pohledu je použit pomocník třídy \emph{CalendarHelper::calendar()}.
...
vybrán databázový systém MySQL, který ve verzi 5.2 úmožňuje fultextové
vyhledánání pouze nad tabulkami typu MyISAM, bylo potřeba zajistit,
...
V~případě, že se náklady odvíjí od počtu zúčastněných osob zašrtněte
...
které tento výraz obsahují (obrázek\vref{fig:Na=000161ept=0000E1va=00010D}).%
\begin{figure}
...
příslušté položky vztahující se k danému produktu. Při splnění všech
těchto podmínek bude smazání úmožněno.
...
tabulce je možné zaškrtnou pouze některé kategorie produktů, ke kterým
byly výkazy přidány.
...
\begin{description}
\item [{Nedostatky}] Pro lepší ochranu uložených dat by se měl zavést ACL
- Access Control List. V současné verzi systému je díky \end{description}
...
\href{http://bakery.cakephp.org/articles/view/p28n-the-top-to-bottom-persistent-internationalization-tutorial}{http://bakery.cakephp.org/articles/view/p28n-the-top-to-bottom-persistent-internationalization-tutorial}

\bibitem{pass-gen}\emph{Random password generator component for CakePHP},
Červen 2008\\
\href{http://www.solitechgmbh.com/2008/06/11/random-password-generator-component-for-cakephp/}{http://www.solitechgmbh.com/2008/06/11/random-password-generator-component-for-cakephp/}

\bibitem{linkable}Rafael Bandeira: \emph{Linkable Behavior. Taking
it easy in your DB}, Listopad 2008\\
\href{http://blog.rafaelbandeira3.com/2008/11/16/linkable-behavior-taking-it-easy-in-your-db/}{http://blog.rafaelbandeira3.com/2008/11/16/linkable-behavior-taking-it-easy-in-your-db/}
